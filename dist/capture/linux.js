...
async function getWaylandCompositor() {
    const desktop = process.env.XDG_CURRENT_DESKTOP?.toLowerCase() ?? "";
    if (desktop.includes("hypr"))
        return "hyprland";
...
async function getActiveWindowWayland() {
...
async function getActiveWindowX11() {
    try {
...
async function captureWayland(target) {
...
async function captureX11(target) {
    const tmpFile = `/tmp/opencode-dc-${Date.now()}.png`;
    if (target === "activeWindow") {
...
async function captureLinux(target) {
    const wayland = process.env.WAYLAND_DISPLAY;
    if (wayland)
        return captureWayland(target);
